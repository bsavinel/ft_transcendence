...
export class PongGateway implements OnGatewayConnection, OnGatewayDisconnect {
	...
	quitMatchmaking(client: Socket): void {
		const player = { id: client.id, socket: client };
		const index = this.queueClassic.findIndex(
			(client) => client.id === player.id
		);
		if (index !== -1) {
			this.queueClassic.splice(index, 1);
			console.log(
				`Player ${client.id} leave the queue for classic Pong.`
			);
		}
	}
	...
	quitMatchmakingPowerUp(client: Socket): void {
		const player = { id: client.id, socket: client };
		const index = this.queuePower.findIndex(
			(client) => client.id === player.id
		);
		if (index !== -1) {
	...
	manageLaunchSpell(
		...
		@MessageBody() data: { roomId: string; side: string }
	): void {
	...
	manageMousePosition(
		...
		if (this.player.has(roomId) && this.bonus.has(roomId)) {
			...
			if (side === 'left') {
				if (
					mousePos.y + bonus.heightPaddleScale < BOARD_HEIGHT &&
					mousePos.y > 0
				)
					player.player1Position = {
						x: player.player1Position.x,
						y: mousePos.y,
					};
				else if (mousePos.y + bonus.heightPaddleScale > BOARD_HEIGHT)
					player.player1Position = {
						x: player.player1Position.x,
						y: BOARD_HEIGHT - bonus.heightPaddleScale,
					};
				...
				if (
					mousePos.y + bonus.heightPaddleScale2 < BOARD_HEIGHT &&
					mousePos.y > 0
				)
					player.player2Position = {
						x: player.player2Position.x,
						y: mousePos.y,
					};
				else if (mousePos.y + bonus.heightPaddleScale2 > BOARD_HEIGHT)
					player.player2Position = {
						x: player.player2Position.x,
						y: BOARD_HEIGHT - bonus.heightPaddleScale2,
					};
				else if (mousePos.y < 0)
					player.player2Position = {
						x: player.player2Position.x,
	...
	startGamePower(
		...
		const { roomId } = data;
		if (
			this.player.has(roomId) &&
			this.ball.has(roomId) &&
			this.bonus.has(roomId) &&
			this.game.has(roomId)
		) {
			const player = this.player.get(roomId);
			...
			this.server.to(roomId).emit('updatePowerUp', {
				player: player,
				ball: ball,
				bonus: bonus,
				game: game,
			...
			if (
				player.player1Score >= SCORE_LIMIT ||
				player.player2Score >= SCORE_LIMIT
			) {
				const side =
					player.player1Score >= SCORE_LIMIT ? 'left' : 'right';
				...
				if (arrayRoom) {
					...
					const player1 = {
						id: player1Id,
						score: player.player1Score,
					};
					const player2 = {
						id: player2Id,
						score: player.player2Score,
					};
					...
					this.players = this.players.filter(
						(players) => players.client !== player1Socket
					);
					this.players = this.players.filter(
						(players) => players.client !== player2Socket
					);
			...
			const distance = Math.sqrt(
				Math.pow(ball.ballPosition.x - bonus.bonusPosition.x, 2) +
					Math.pow(ball.ballPosition.y - bonus.bonusPosition.y, 2)
			);
			const distance2 = Math.sqrt(
				Math.pow(ball.ballPosition.x - bonus.bonusPosition2.x, 2) +
					Math.pow(ball.ballPosition.y - bonus.bonusPosition2.y, 2)
			);
				...
				if (bonus.showBonus) {
					if (distance <= BONUS_WIDTH / 2 + BONUS_HEIGHT / 2) {
						...
						if (player.whichSpellPlayer1 === null)
							player.whichSpellPlayer1 = Object.assign(
								{},
								bonus.selectedSpell
							);
						bonus.bonusPosition = {
						...
						bonus.selectedSpell =
							spells[Math.floor(Math.random() * 4)];
						bonus.timeLeft = TIME_LEFT;
						socket.to(roomId).emit('bonusAudio');
					}
				} else {
					setTimeout(() => {
						bonus.showBonus = true;
					}, TIME_LEFT_BLOC_BONUS * 1000);
				}
				if (bonus.showBonus2) {
					if (distance2 <= BONUS_WIDTH / 2 + BONUS_HEIGHT / 2) {
						...
						if (player.whichSpellPlayer1 === null)
							player.whichSpellPlayer1 = Object.assign(
								{},
								bonus.selectedSpell2
							);
						bonus.bonusPosition2 = {
							x: BOARD_WIDTH * 0.75,
							y: -BONUS_HEIGHT,
						};
						bonus.selectedSpell2 =
				...
				if (bonus.showBonus) {
					if (distance <= BONUS_WIDTH / 2 + BONUS_HEIGHT / 2) {
						...
						if (player.whichSpellPlayer2 === null)
							player.whichSpellPlayer2 = Object.assign(
								{},
								bonus.selectedSpell
							);
						bonus.bonusPosition = {
			...
			// hitScenery();
			if (
				ball.ballPosition.y - BALL_RADIUS <= 0 &&
				ball.ballDirection.y < 0
			) {
				// Touche le plafond
				ball.ballDirection = {
					x: ball.ballDirection.x,
					y: -ball.ballDirection.y,
				};
				socket.to(roomId).emit('wallsAudio');
			} else if (
				ball.ballPosition.y + BALL_RADIUS >= BOARD_HEIGHT &&
				ball.ballDirection.y > 0
			) {
				// Touche le sol
				ball.ballDirection = {
				...
				ball.ballPosition.x - BALL_RADIUS <= 0 &&
				ball.ballDirection.x < 0
			) {
				...
				ball.ballPosition.x + BALL_RADIUS >= BOARD_WIDTH &&
				ball.ballDirection.x > 0
			) {
				...
				ball.ballPosition.x - BALL_RADIUS <=
					player.player1Position.x + PADDLE_WIDTH &&
				ball.ballPosition.y >= player.player1Position.y &&
				ball.ballPosition.y <=
					player.player1Position.y + bonus.heightPaddleScale &&
				ball.ballDirection.x < 0
			) {
				if (ball.ballPower) {
					ball.ballPower = false;
				...
				if (
					ball.ballPosition.y > player.player1Position.y &&
					ball.ballPosition.y <
						player.player1Position.y + bonus.heightPaddleScale / 5
				) {
					ball.ballDirection = { x: 1, y: Math.random() * -2.5 };
				} else if (
					ball.ballPosition.y >=
						player.player1Position.y +
							bonus.heightPaddleScale / 5 &&
					ball.ballPosition.y <
						player.player1Position.y +
							(bonus.heightPaddleScale / 5) * 2
				) {
				...
				if (player.player1ActivateBonus) {
					if (player.whichSpellPlayer1?.name == 'Super strike') {
						...
						ball.ballSpeed =
							BALL_SPEED * player.whichSpellPlayer1?.speed;
						ball.ballPower = true;
				...
				ball.ballPosition.x + BALL_RADIUS >= player.player2Position.x &&
				ball.ballPosition.y >= player.player2Position.y &&
				ball.ballPosition.y <=
					player.player2Position.y + bonus.heightPaddleScale2 &&
				ball.ballDirection.x > 0
			) {
				if (ball.ballPower) {
					ball.ballPower = false;
				...
				if (
					ball.ballPosition.y > player.player2Position.y &&
					ball.ballPosition.y <
						player.player2Position.y + bonus.heightPaddleScale2 / 5
				) {
					ball.ballDirection = { x: -1, y: Math.random() * -2.5 };
				} else if (
					ball.ballPosition.y >=
						player.player2Position.y +
							bonus.heightPaddleScale2 / 5 &&
					ball.ballPosition.y <
						player.player2Position.y +
							(bonus.heightPaddleScale2 / 5) * 2
				) {
					ball.ballDirection = { x: -1, y: Math.random() * -2 };
				} else if (
					ball.ballPosition.y >=
				...
				if (player.player2ActivateBonus) {
					if (player.whichSpellPlayer2?.name == 'Super strike') {
						ball.ballDirection = { x: -1, y: 0 };
						ball.ballSpeed =
							BALL_SPEED * player.whichSpellPlayer2?.speed;
						ball.ballPower = true;
				...
				ball.ballPosition = {
					x:
						ball.ballPosition.x +
						ball.ballDirection.x * ball.ballSpeed,
					y:
						ball.ballPosition.y +
						ball.ballDirection.y * ball.ballSpeed,
				};
			}
			...
			if (bonus.showBonus) {
				if (bonus.bonusPosition.y <= BOARD_HEIGHT)
					bonus.bonusPosition = {
						x: bonus.bonusPosition.x,
						y: bonus.bonusPosition.y + BONUS_SPEED,
					};
				else {
					bonus.bonusPosition = {
						x: BOARD_WIDTH * 0.25,
			...
			if (bonus.showBonus2) {
				if (bonus.bonusPosition2.y <= BOARD_HEIGHT)
					bonus.bonusPosition2 = {
						x: bonus.bonusPosition2.x,
						y: bonus.bonusPosition2.y + BONUS_SPEED,
					};
				else {
					bonus.bonusPosition2 = {
						x: BOARD_WIDTH * 0.75,
	...
	startGame(
		...
		if (
			...
			if (
				...
				ball.ballPosition.y >= player.player1Position.y &&
				ball.ballPosition.y <=
					player.player1Position.y + PADDLE_HEIGHT &&
				ball.ballDirection.x < 0
			) {
				ball.ballPower = false;
				ball.ballSpeed += INCREASE_SPEED;
				if (
					ball.ballPosition.y > player.player1Position.y &&
					ball.ballPosition.y <
						player.player1Position.y + PADDLE_HEIGHT / 5
				) {
					ball.ballDirection = { x: 1, y: Math.random() * -2.5 };
				} else if (
					ball.ballPosition.y >=
						player.player1Position.y + PADDLE_HEIGHT / 5 &&
					ball.ballPosition.y <
						player.player1Position.y + (PADDLE_HEIGHT / 5) * 2
				) {
					ball.ballDirection = { x: 1, y: Math.random() * -2 };
				} else if (
					ball.ballPosition.y >=
				...
				ball.ballPosition.y >= player.player2Position.y &&
				ball.ballPosition.y <=
					player.player2Position.y + PADDLE_HEIGHT &&
				ball.ballDirection.x > 0
			) {
				ball.ballPower = false;
				ball.ballSpeed += INCREASE_SPEED;
				if (
					ball.ballPosition.y > player.player2Position.y &&
					ball.ballPosition.y <
						player.player2Position.y + PADDLE_HEIGHT / 5
				) {
					ball.ballDirection = { x: -1, y: Math.random() * -2.5 };
				} else if (
					ball.ballPosition.y >=
						player.player2Position.y + PADDLE_HEIGHT / 5 &&
					ball.ballPosition.y <
						player.player2Position.y + (PADDLE_HEIGHT / 5) * 2
				) {
					ball.ballDirection = { x: -1, y: Math.random() * -2 };
				} else if (
					ball.ballPosition.y >=
	...
	protected async launchInvitGame(
		...
		if (!this.invitMap.has(invitId)) {
			...
			player1.socket.join(room);
			player2.socket.join(room);
			player1.socket.to(room).emit('launchOn', true); //a mettre dans le front
			player2.socket.to(room).emit('launchOn', true);
